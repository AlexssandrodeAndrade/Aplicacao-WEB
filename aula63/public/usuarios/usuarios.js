...
async function requisicao(url, opcoes = {}) {
  ...
  if (!resposta.ok) throw new Error(dados.mensagem || 'Erro na requisição.');
  return dados;
}
...
function renderizar() {
  ...
  document.querySelector('#quantidade-resultados').textContent = `${filtrados.length} resultado(s)`;
  lista.innerHTML = filtrados.length ? filtrados.map((usuario) => `
    <tr>
      <td><span class="avatar">${usuario.nome.split(' ').map(p => p[0]).slice(0, 2).join('')}</span><span><strong>${usuario.nome}</strong><small>ID #${String(usuario.id).padStart(3, '0')}</small></span></td>
      <td>${usuario.email}</td>
      <td><button class="acao" data-editar="${usuario.id}" aria-label="Editar ${usuario.nome}">✎</button><button class="acao excluir" data-excluir="${usuario.id}" aria-label="Excluir ${usuario.nome}">×</button></td>
    </tr>`).join('') : '<tr><td class="vazio" colspan="3">Nenhum usuário encontrado.</td></tr>';
}
...
lista.addEventListener('click', async (evento) => {
  ...
  if (idExcluir) {
    ...
    try {
      const dados = await requisicao(`/usuarios/${idExcluir}`, { method: 'DELETE' });
      mostrarMensagem(dados.mensagem);
      await carregarUsuarios();
    } catch (erro) { mostrarMensagem(erro.message, true); }
  }
